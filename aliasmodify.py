...
def find_index_of_user(arr, match_string):
    ...
    try:
        ...
        if len(dict_users_index) > 1:
            ...
            print(f"\nPROMPT: Found {len(dict_users_index)} matching profile for {match_string}")
            for index, user in enumerate(dict_users_index): 
                if "entry added" not in user: print(f"Index {index}: {user}")
            
            # Select index from the list of matching users
            chosen_index = int(input(f"PROMPT: Choose the index for profile of {match_string}: "))
            print("\n")
    ...
    except: print(f"{Fore.RED}{Style.BRIGHT}ERROR{Style.NORMAL}{Fore.RESET}: {match_string} not found")
...
def find_address_of_user(address_arr, user_index):
    try:
        ...
        return user_address
    except: print(f"{Fore.RED}{Style.BRIGHT}ERROR{Style.NORMAL}{Fore.RESET}: Address cannot be determined")
    

def modify_address_of_user(user_index, whitelist_address):
    try: 
        address_modified = whitelist_address
        ...
        if valid_ip:
            ...
            for index, address in enumerate(address_arr):
                if user_index == index: 
                    if address_arr[index] != address_modified: address_arr[index] = address_modified
                    else: continue
        ...
        else: print(f"{Fore.RED}{Style.BRIGHT}ERROR{Style.NORMAL}{Fore.RESET}: Enter a valid IP Address")
    except: print(f"{Fore.RED}{Style.BRIGHT}ERROR{Style.NORMAL}{Fore.RESET}: Address cannot be modified")
